...
def gpulidar_airpub(frameID, pubNode, sensorName, vehicleName):
    ...
    while not rospy.is_shutdown():
        ...
        # get lidar data
        lidarData = client.getGPULidarData(sensorName, vehicleName)
...
if __name__ == '__main__':
    try:
        rospy.init_node('airsim_gpulidar', anonymous=True)
        frameID = rospy.get_param('~frame_id', 'base_laser')
        pubNode =  rospy.get_param('~pub_node', 'airsim/lidar')
        sensorName =  rospy.get_param('~sensor_name', 'gpulidar')
        vehicleName = rospy.get_param('~vehicle_name', 'airsimvehicle')
        gpulidar_airpub(frameID, pubNode, sensorName, vehicleName)
