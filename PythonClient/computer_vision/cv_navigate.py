...
class AvoidLeft(AbstractClassGetNextVec):
    ...
    def get_next_vec(self, depth, obj_sz, goal, pos):
        ...
        img2d_box = img2d[
            int((h - roi_h) / 2) : int((h + roi_h) / 2), int((w - roi_w) / 2) : int((w + roi_w) / 2)
        ]
...
def moveUAV(client, pos, yaw):
    client.simSetVehiclePose(
        airsim.Pose(airsim.Vector3r(pos[0], pos[1], pos[2]), airsim.to_quaternion(0, 0, yaw)), True
    )
...
responses = client.simGetImages([airsim.ImageRequest("1", airsim.ImageType.DepthPlanar, True)])
