...
if __name__ == "__main__":
    ...
    with open(
        "airsim_segmentation_colormap_list_"
        ...
        for index, item in enumerate(currentObjectList):
            f.write("{},{}\n".format(item, ",".join([str(x) for x in colorMap[index, :]])))
    print(
        "Generated list of all current objects with their RGB value with a total of "
        + str(len(currentObjectList))
        + " objects\n"
    )
    ...
    with open(
        "airsim__poses_list_" + datetime.now().strftime("%Y_%m_%d_%H_%M_%S") + ".csv", "w"
    ) as f:
        f.write("ObjectName,x_pos,y_pos,z_pos,w_qua,x_qua,y_qua,z_qua\n")
        for index, item in enumerate(currentObjectList):
            currentObjectPose = currentPosesList[index]
            currentObjectPoseString = (
                str(currentObjectPose.position.x_val)
                + ","
            ...
            f.write(f"{item},{currentObjectPoseString}\n")
    print(
        "Generated list of all current objects with their poses a total of "
        + str(len(currentObjectList))
        + " objects\n"
    )
    ...
    with open("segmentation.csv", encoding="utf-8-sig") as csv_file:
        ...
        for idx, row in enumerate(csv_reader, start=1):
            ...
            print(
                "Found "
                + str(len(classItems))
                + " objects starting with object-name '"
                + str(row[0])
                + "'"
            )
    ...
    print(
        "Setting all objects in world matching class-name '"
        + className
        + "' a certain color, based on object list..."
    )
    for item in classObjectMap[className]:
        success = client.simSetSegmentationObjectID(item, classColorIndex, False)
        if success:
            print(
                "Found object matching object-name '"
                + item
    ...
    print(
        "Found and changed color on all objects in world matching class-name '"
        + className
        + "' based on object list\n"
    )
    # b) In this version we set it based on regex of the classname
    print(
        "Setting all objects in world matching class-name '"
        + className
        + "' a certain color, based on regex..."
    )
    success = client.simSetSegmentationObjectID(className + ".*", classColorIndex, True)
    if success:
        print(
            "Found objects matching object-name '"
            + className
            + "'! Set segmentation value "
            + str(classColorIndex)
        )
    ...
    print(
        "Found RGB color associated with random object '"
        + randomObjectName
        + "':"
        + str(randomObjectColor)
        + "\n"
    )
    ...
    responses = client.simGetImages(
        [airsim.ImageRequest("frontcamera", airsim.ImageType.Segmentation, False, False)]
    )
