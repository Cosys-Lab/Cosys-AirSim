...
for i in range(0, len(uniqueColors)):
    FoundIndex = np.where(
        (colormap == np.array([uniqueColors[i], uniqueColors[i], uniqueColors[i]])).all(axis=1)
    )[0]
