...
class DriveDataGenerator(image.ImageDataGenerator):
    ...
    def random_transform_with_states(self, x, seed=None):
        ...
        if tx != 0 or ty != 0:
            ...
            transform_matrix = (
                shift_matrix if transform_matrix is None else np.dot(transform_matrix, shift_matrix)
            )

        if shear != 0:
            shear_matrix = np.array([[1, -np.sin(shear), 0], [0, np.cos(shear), 0], [0, 0, 1]])
            transform_matrix = (
                shear_matrix if transform_matrix is None else np.dot(transform_matrix, shear_matrix)
            )

        if zx != 1 or zy != 1:
            zoom_matrix = np.array([[zx, 0, 0], [0, zy, 0], [0, 0, 1]])
            transform_matrix = (
                zoom_matrix if transform_matrix is None else np.dot(transform_matrix, zoom_matrix)
            )

        if transform_matrix is not None:
            h, w = x.shape[img_row_axis], x.shape[img_col_axis]
            transform_matrix = image.transform_matrix_offset_center(transform_matrix, h, w)
            x = image.apply_transform(
                x, transform_matrix, img_channel_axis, fill_mode=self.fill_mode, cval=self.cval
            )
        ...
        if self.brighten_range != 0:
            random_bright = np.random.uniform(
                low=1.0 - self.brighten_range, high=1.0 + self.brighten_range
            )
...
class DriveIterator(image.Iterator):
    ...
    def __init__(
        ...
        super().__init__(x_images.shape[0], batch_size, shuffle, seed)
    ...
    def __get_indexes(self, index_array):
        ...
        if self.x_prev_states is not None:
            batch_x_images = np.zeros(
                tuple([self.batch_size] + list(self.x_images.shape)[1:]), dtype=K.floatx()
            )
            batch_x_prev_states = np.zeros(
                tuple([self.batch_size] + list(self.x_prev_states.shape)[1:]), dtype=K.floatx()
            )
        else:
            batch_x_images = np.zeros(
                tuple([self.batch_size] + list(self.x_images.shape)[1:]), dtype=K.floatx()
        ...
        if self.save_to_dir:
            for i in range(0, self.batch_size, 1):
                ...
                fname = f"{self.save_prefix}_{1}_{hash}.{self.save_format}"
                img.save(os.path.join(self.save_to_dir, fname))

        batch_y = self.y[sorted(used_indexes)]
        idx = []
