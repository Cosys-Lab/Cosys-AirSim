    ...
    for folder in folders:
        ...
        for i in range(1, current_df.shape[0] - 1):
            ...
            image_filepath = os.path.join(
                os.path.join(folder, "images"), current_df.iloc[i]["ImageName"]
            ).replace("\\", "/")
...
def generatorForH5py(data_mappings, chunk_size=32):
    ...
    for chunk_id in range(0, len(data_mappings), chunk_size):
        ...
        if len(data_chunk) == chunk_size:
            ...
            yield (
                image_names_chunk,
                labels_chunk.astype(float),
                previous_state_chunk.astype(float),
            )
...
def saveH5pyData(data_mappings, target_file_path, chunk_size):
    ...
    with h5py.File(target_file_path, "w") as f:
        ...
        dset_images = f.create_dataset(
            "image",
            shape=images_chunk.shape,
            maxshape=images_chunk_maxshape,
            chunks=images_chunk.shape,
            dtype=images_chunk.dtype,
        )

        dset_labels = f.create_dataset(
            "label",
            shape=labels_chunk.shape,
            maxshape=labels_chunk_maxshape,
            chunks=labels_chunk.shape,
            dtype=labels_chunk.dtype,
        )

        dset_previous_state = f.create_dataset(
            "previous_state",
            shape=previous_state_chunk.shape,
            maxshape=previous_state_maxshape,
            chunks=previous_state_chunk.shape,
            dtype=previous_state_chunk.dtype,
        )
