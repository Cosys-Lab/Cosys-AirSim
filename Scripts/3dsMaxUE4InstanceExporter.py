 ...
 def execute_export(self):
	...
	try:		
		...
		if objectCount is not 0:
			...
			self.executeInfoLabel.setText("Exporting...\n 0/" + str(objectCount) + " completed.")
			...
			for each in mySel:
				index = index + 1
				self.executeInfoLabel.setText("Exporting...\n" + str(index) + "/" + str(objectCount) + " completed.")
				print("Adding object", each.Name)
				
				objName = each.Name
				objPosX = round(each.Position.GetX(), 2) + 0
				objPosY = round(each.Position.GetY() * -1, 2) + 0
				objPosZ = round(each.Position.GetZ(), 2) + 0
				objRotX = round(each.Rotation.GetEuler().GetY() * -1 * 180 / math.pi , 4) + 0
				objRotY = round(each.Rotation.GetEuler().GetZ() * -1 * 180 / math.pi , 4) + 0
				objRotZ = round(each.Rotation.GetEuler().GetX() * 180 /  math.pi , 4) + 0
				
				print("      Begin Actor Class=StaticMeshActor Name={} Archetype=StaticMeshActor'/Script/Engine.Default__StaticMeshActor'".format(objName.encode("utf-8")), file = instancesFile)
				...
				print("            RelativeLocation=(X={},Y={},Z={})".format(objPosX, objPosY, objPosZ), file = instancesFile)
				print("            RelativeScale3D=(X=1.0,Y=1.0,Z=1.0)", file = instancesFile)
				print("            RelativeRotation=(Pitch={},Yaw={},Roll={})".format(objRotX, objRotY, objRotZ), file = instancesFile)
				print("            CustomProperties \n         End Object\n         StaticMeshComponent=StaticMeshComponent0\n         Components(0)=StaticMeshComponent0\n         RootComponent=StaticMeshComponent0", file = instancesFile)
				print("         ActorLabel=\"{}\"".format(objName.encode("utf-8")), file = instancesFile)
